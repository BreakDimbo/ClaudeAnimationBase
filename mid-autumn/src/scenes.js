...
// Each character is merged into one silhouette first (strokes pressed in at the start, a small press at each turn,
// horizontals thinner than verticals), then painted as one wash: colour, granulation, pigment pooled at the edge.
// k: gold 0..1; o.only: 'ri' draws just the 日 of 阳; o.except/exceptA leave it out; o.fade multiplies everything
const GLY = new Map();
function glyphMask(ch, size, filt) {
  const key = ch + size + filt; let c = GLY.get(key); if (c) return c;
  const pad = size * .25, S = size / 100; c = document.createElement('canvas'); c.width = c.height = Math.ceil(size + pad * 2);
  const g = c.getContext('2d'); g.translate(pad, pad); g.fillStyle = g.strokeStyle = '#FFFFFF'; g.lineCap = 'round'; g.lineJoin = 'round';
  GLYPH[ch].forEach(st => {
    const tag = typeof st[st.length - 1] === 'string' ? st[st.length - 1] : null;
    if (filt === 'only' && tag !== 'ri') return; if (filt === 'except' && tag === 'ri') return;
    const nums = st.filter(v => typeof v === 'number'), P = st.filter(v => Array.isArray(v)).map(p => [p[0] * S, p[1] * S]), w0 = nums[0] * S * 1.4, w1 = nums[1] * S * 1.4;
    for (let j = 0; j < P.length - 1; j++) {
      const [p, q] = [P[j], P[j + 1]], horiz = Math.abs(q[0] - p[0]) > Math.abs(q[1] - p[1]) * 1.5, f = horiz ? .78 : 1.08;
      const a0 = lerp(w0, w1, j / (P.length - 1)) * f, a1 = lerp(w0, w1, (j + 1) / (P.length - 1)) * f;
      const n = 12; for (let i = 0; i < n; i++) { const u0 = i / n, u1 = (i + 1) / n; g.lineWidth = lerp(a0, a1, (u0 + u1) / 2) * (1 + .12 * Math.sin(u0 * Math.PI)); g.beginPath(); g.moveTo(lerp(p[0], q[0], u0), lerp(p[1], q[1], u0) - (horiz ? Math.sin(u0 * Math.PI) * a0 * .12 : 0)); g.lineTo(lerp(p[0], q[0], u1), lerp(p[1], q[1], u1) - (horiz ? Math.sin(u1 * Math.PI) * a0 * .12 : 0)); g.stroke(); }
      if (j > 0) { g.save(); g.translate(p[0] + a0 * .1, p[1] + a0 * .08); g.rotate(.7); g.beginPath(); g.ellipse(0, 0, a0 * .6, a0 * .46, 0, 0, TAU); g.fill(); g.restore(); }
    }
    g.save(); g.translate(P[0][0] + w0 * .08, P[0][1] + w0 * .05); g.rotate(.75); g.beginPath(); g.ellipse(0, 0, w0 * .62, w0 * .46, 0, 0, TAU); g.fill(); g.restore();
  });
  GLY.set(key, c); return c;
}
function glyphPaint(ch, size, filt, col) {
  const key = ch + size + filt + col; let c = GLY.get(key); if (c) return c;
  const m = glyphMask(ch, size, filt), w = m.width, h = m.height; c = document.createElement('canvas'); c.width = w; c.height = h;
  const g = c.getContext('2d'), r = mulberry(skey(key));
  g.fillStyle = col; g.fillRect(0, 0, w, h);
  for (let i = 0; i < w * h / 60; i++) { g.fillStyle = rgba(mix(col, i % 3 ? '#5A3A10' : '#FFF4C8', .5), .05 + r() * .12); g.fillRect(r() * w, r() * h, 1 + r() * 1.5, 1 + r() * 1.5); }
  const e = document.createElement('canvas'); e.width = w; e.height = h; const eg = e.getContext('2d');       // pooled edge
  eg.fillStyle = mix(col, '#3A2208', .45); eg.fillRect(0, 0, w, h); eg.globalCompositeOperation = 'destination-out'; eg.filter = `blur(${Math.max(1, size * .018).toFixed(1)}px)`; eg.drawImage(m, 0, 0);
  g.globalAlpha = .5; g.drawImage(e, 0, 0); g.globalAlpha = 1;
  g.globalCompositeOperation = 'destination-in'; g.filter = `blur(${Math.max(.5, size * .004).toFixed(2)}px)`; g.drawImage(m, 0, 0); g.filter = 'none';
  GLY.set(key, c); return c;
}
function glyph(key, ch, cx, cy, size, k, o = {}) {
  const filt = o.only ? 'only' : (o.except && (o.exceptA ?? 1) <= 0 ? 'except' : 'all'), fade = o.fade ?? 1;
  if (fade <= 0) return;
  const pad = size * .25, x = cx - size / 2 - pad, y = cy - size / 2 - pad;
  ctx.save(); ctx.globalAlpha *= fade;
  if (o.dim !== false) { ctx.globalAlpha *= .9; ctx.drawImage(glyphPaint(ch, size, filt, o.dimCol || '#7A5C34'), x, y); ctx.globalAlpha /= .9; }
  if (k > 0) { ctx.globalAlpha *= clamp(k); ctx.drawImage(glyphPaint(ch, size, filt, '#F0C04C'), x, y); }
  ctx.restore();
  if (k > 0 && o.glow !== false) wcGlow(cx, cy, size * .9, '#FFD27A', .18 * clamp(k) * fade);
}
...
function gateLayer() {
  layer('gate', -400, -300, 2720, 1700, () => {
    ...
    // tile rows (瓦垄): even channels, alternately lit, with round tile ends (瓦当) along the eave
    for (let i = 0, x = 400; x <= 1520; x += 20, i++) {
      const u0 = (x - 400) / 1120, u1 = (x + 20 - 400) / 1120, top0 = lerp(476, 1444, u0), top1 = lerp(476, 1444, u1), b0 = x + (x - 960) * .06, b1 = x + 20 + (x + 20 - 960) * .06;
      ctx.fillStyle = i % 2 ? 'rgba(58,62,78,.55)' : 'rgba(92,98,116,.45)'; ctx.beginPath(); ctx.moveTo(top0, 222); ctx.lineTo(top1, 222); ctx.lineTo(b1, 344); ctx.lineTo(b0, 344); ctx.closePath(); ctx.fill();
      if (i % 2) { ctx.fillStyle = 'rgba(40,44,58,.8)'; ctx.beginPath(); ctx.arc((b0 + b1) / 2, 346, 7, 0, TAU); ctx.fill(); }
    }
...
function hutongLayer() {
  layer('hutong', -600, -300, 4400, 1700, () => {
    ...
    for (let i = 0; i < 5; i++) { const x = -100 + i * 900; wcStroke('htsh' + i, [[x, 380], [x + 120, 600], [x + 160, 900]], 22, 8, '#7A7AA0', { a: .03, wet: true }); for (let j = 0; j < 22; j++) wcAt('htsl' + i + ':' + j, UNIT(8), '#7A7AA0', x - 180 + hash('hs', i, j) * 560, 390 + hash('hs', j, i) * 300, 12 + hash('hs', i + j) * 22, { sy: .55, rot: hash('hs', j) * 3, a: .03, wet: true }); }
