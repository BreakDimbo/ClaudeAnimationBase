...
function S2(lt, t) {
  ...
  if (lt < 3) {                                   // 10–13 the drop rolls in and bounces at his feet
    ...
    puppet('rere', 'main', 1180, 1010, 800, { t });
    const k = seg(lt, 0, 2.4), x = lerp(-60, 930, easeOut(k)), b = hop(lt, 2.3, 2.75, 70);
    ...
    camEnd();
  } else {                                        // 19–22 tilt down his body; the drop slides; his jeans bleed into a river
    const cy = kf(lt, [[9, 420], [12, 900]], easeIO);
    camBegin(1060, cy, 1.25);
    sky('s2sky', blue, { ground: '#C9D3DF', gy: 820 });
    puppet('rere', 'main', 1060, 1010, 800, { t });
...
function S3(lt, t) {
  ...
  if (lt < 7) {                                   // 22–25 the boat drifts in (pan); 25–29 his stripes become keys
    ...
    puppet('wo', 'main', bx, 930, 640, { t, rot: rock, shadow: false });
    paperBoat(bx, 950, 200, rock);
    const peel = seg(lt, 3.2, 4.6);
    for (let i = 0; i < KEYS; i++) {
      const k = clamp(peel * 1.6 - i * .12), sx = bx + 10, sy = 560 + i * 24;
    ...
    if (lt > 4.6) {
      ...
      drop(x, y, 20, t);
    } else drop(bx + 130, 860, 20, t);
    rabbit(bx + 110, 915, 36, t, { alpha: .95 });
    camEnd();
  } else if (lt < 10) {                           // 29–32 his face: a smile
    const z = kf(lt, [[7, 1], [10, 1.08]], ease);
    camBegin(960, 540, z);
    riverBG('s3r');
    moonWindow('s3w', 'wo', 'e0', 'e1', seg(lt, 7.8, 8.6), 960, 520, 360, '#6F7FB8', t, { size: 1.75, bottom: 1.08 });
    camEnd();
...
function S5(lt, t) {
  ...
  const scene = (grow, shift) => {
    ...
    puppet('tongtong', 'front', TX + 60 + shift * 200, 1060, 880, { t, rot: shift * .05 });
  };
  if (lt < 3) {                                   // 46–49 tilt up: the pattern on his sweater grows into the tree
    ...
    camBegin(960, 330, z);
    scene(1, 1);
    moonWindow('s5w', 'lulu', 'e0', 'e1', seg(lt, 9.6, 10.4), 1400, 120, 230, '#F3D9A4', t, { size: 1.85, bottom: 1.08 });
...
// ═════════ 镜9 黄昏的路 (106–124) · 橙紫 ═════════
const WALK = ['laolao', 'rere', 'tongtong', 'lulu', 'daidai', 'doudou', 'wo', 'mama', 'baba', 'eryi', 'sanyi', 'erjiumu', 'erjiu', 'daju'];
...
const TABLE_FRONT = ['rere', 'daidai', 'wo', 'lulu', 'tongtong', 'doudou'];
...
function S10(lt, t) {
  ...
  if (lt < 4) { camBegin(960, 540, 1); drawTable({ seat: 0 }); camEnd(); }
    ...
    const ALL = ['laoye', 'laolao', 'daju', 'mama', 'baba', 'erjiu', 'erjiumu', 'eryi', 'sanyi', 'sanyifu', 'wo', 'lulu', 'tongtong', 'rere', 'daidai', 'doudou'];
...
const PHOTO = [
  ...
  ['daidai', 440, 800], ['lulu', 640, 790], ['tongtong', 860, 770], ['doudou', 1080, 780], ['eryi', 1300, 780], ['rere', 1500, 790], ['wo', 1700, 800],
];
function S11(lt, t) {
  ...
  if (lt < 8) {                                   // 148–152 they squeeze in, the timer blinks; 152–156 "I" run in last
    ...
    const backs = ['erjiumu', 'sanyi', 'daidai', 'sanyifu', 'lulu', 'mama', 'tongtong', 'laoye', 'doudou', 'laolao', 'eryi', 'baba', 'rere', 'daju', 'wo', 'erjiu'];
...
const SHOTS = [
  [0, S1, '纸'], [10, S2, '热热'], [22, S3, '我'], [34, S4, '兜兜袋袋'], [46, S5, '桐桐璐璐'], [58, S6, '二姨'],
